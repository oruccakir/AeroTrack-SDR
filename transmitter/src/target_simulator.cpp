...
void TargetSimulator::clearTargets()
{
    targets_.clear();
}

TargetSimulator::IQVector TargetSimulator::simulatePulse(uint32_t pulse_index) const
{
    ...
    for (const auto& target : targets_) {
        ...
        echo = signal_gen_.applyAmplitudeScaling(echo, target.rcs_m2, target.range_m);
        ...
        double inter_pulse_phase = 2.0 * M_PI * f_doppler * pulse_index * config_.pri_s();

        IQSample phase_factor(
            static_cast<float>(std::cos(inter_pulse_phase)),
